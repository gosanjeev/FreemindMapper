...
object CreateMaps {
  ...
  def recursiveList(f: File, initRef: String, lvl: Int, index: Int, filter: List[String]): scala.xml.Elem = {
    ...
    def folded(f: File): String = {
      if (lvl >= 2 && f.isDirectory()) "true" else null
    }

    node = <node COLOR={color} CREATED={epochTimeMillis} FOLDED={folded(f)} ID={idGen} POSITION={position} LINK={escaper(linkAbs)} MODIFIED={f.lastModified().toString} STYLE="bubble" TEXT={f.getName()}>
      <edge COLOR={color} STYLE={style} WIDTH={width}/>{if (f.isDirectory()) for ((file, idx) <- f.listFiles.zipWithIndex) yield recursiveList(file, initRef, lvl + 1, idx, filter)}
    </node>
    return node
  }
  ...
  def createMindMaps(rootFolder: String, outputFile: String, filter: List[String]) = {
    ...
    val level = 1
    val xml = <map version="1.0.1">
      {recursiveList(file, rootFolder, level, 0, filter)}
    </map>
