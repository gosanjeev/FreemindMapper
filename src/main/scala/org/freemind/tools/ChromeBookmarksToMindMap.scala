...
object ChromeBookmarksToMindMap {
  ...
  def createMindMaps(rootFolder: String, outputFile: String, filter: List[String]) = {
    ...
    val r = new Random()
    val level = 1
    var xml = <map version="1.0.1"> { recursiveList(file, rootFolder, level, 0, filter) } </map>
  ...
  def main(args: Array[String]) {
    ...
    val filter =  List()
    createMindMaps("~/Library/Application Support/Google/Chrome/Profile 1", "/Users/sanjeev/Dropbox/MindMaps/Projects/Bookmarks-BV.mm", filter)
